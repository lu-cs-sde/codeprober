...
setupfile="../../website/setup.sh"
if [ ! -f "$setupfile" ]; then
  echo "Missing '$setupfile'. Go to the 'website' dir, copy 'setup.sh.example' to 'setup.sh' and fill in with your own configuration"
  exit 1
fi
...
aws s3 sync $WETNESS --delete $SRC_WEBSITE_DIR s3://$S3_BUCKET_NAME/docs
echo "S3 sync done"
